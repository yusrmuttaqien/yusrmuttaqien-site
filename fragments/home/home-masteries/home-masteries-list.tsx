...
const styles = {
  firstParagraph: 'body-normal mb-[.8lh]',
...
export default function HomeMasteriesList({ title, contents, idx }: MasteriesListProps) {
  return (
    <article
      data-framer={`home-masteries-list-${idx}`}
    ...
    >
      <h3 data-framer={`home-masteries-list-title-${idx}`} className="body-normal uppercase">
        {title}
      </h3>
      <div
        data-framer={`home-masteries-list-contents-${idx}`}
        className="space-y-[.5lh] lg:w-[clamp(24.4375rem,_25.1387rem_+_-1.3453vw,_24.0625rem)] shrink-0"
      >
        {contents.map((content, innerIdx) => (
          <p
            data-framer={`home-masteries-list-content-${idx}-${innerIdx}`}
