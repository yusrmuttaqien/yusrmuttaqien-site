...
import { TIMELINE_ENTRY } from '@/components/pages/projects/Lists/constant';
...
export default function useEntry() {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    function _startSequence(complete: boolean = false) {
      const prepare = animate(TIMELINE_ENTRY(scope).invisible);
      resumables.current.status = 'preparing';
      prepare.then(() => {
        root.classList.remove('invisible');

        resumables.current.instance = animate(TIMELINE_ENTRY(scope).visible);
